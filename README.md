...
* [ ] `bonuslimit` — количество пикч, которые надо тянуть с дёрпибуры для бонусных спойлеров. Подразумевается, что будет вытащено несколько пикч (по умолчанию 3), и для них будет сделано несколько бонусных спойлеров, а потом при отправке поста из них будет выбрана одна самая годная. Если бонус не надо делать, то нужно установить здесь 0.
