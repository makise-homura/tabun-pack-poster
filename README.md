...
* [x] `bonuspony` — дёрпибурной тег бонусной поньки (будет несколько "бонусных" спойлеров в конце). Поиск для бонусной поньки ведётся с исключением основной; то есть, если `pony = 'lyra heartstrings'` и `bonuspony = 'bon-bon'`, то в бонусы будут попадать те картинки, на которых есть Бон-Бон, но нету Лиры.
...
* [ ] `bonuslimit` — количество пикч, которые надо тянуть с дёрпибуры для бонусных спойлеров. Подразумевается, что будет вытащено несколько пикч (по умолчанию 3), и для них будет сделано несколько бонусных спойлеров, а потом при отправке поста из них нужно будет выбрать одну самую годную. Если бонус не надо делать, то нужно установить в 0.
