...
```
pip3 install datetime pathlib http3 urllib3 python-dateutil emoji
...
3. Сконфигурировать скрипт, скопировав файл `config.py.example` в `config.py` и отредактировав последний. Те параметры, которые нельзя оставить, как есть, и нужно обязательно задать по сравнению с дефолтной конфигурацией, выделены далее галочками. В `config.py.example` приведён пример настройки на Лиру и Бон-Бон; обычно требуется конфигурировать все незакомментированные там параметры.

* [ ] `tabun_host` — используемое зеркало табуна.
* [x] `username` — логин на табуне.
* [x] `password` — пароль от табуна.
* [ ] `proxy` — `None` (использовать системный прокси), пустая строка (подключаться без прокси) или строка прокси вида `scheme://[user[:pass]@]host:port`. Можно задать словарём с ключами `'tabun'`, `'booru'`, `'dpaste'` и `'rentry'` отдельно прокси для табуна, буры и остальных ресурсов. Поддерживаются схемы `http://` и `socks5://`, для `'booru'` и `'dpaste'` также `socks5h://`.
* [ ] `mirror` — адрес буры.
...
* [ ] `blog_id` — численный идентификатор блога, куда постить или строка из URL ссылки на блог (например, ЯРОК — это `fanart`, ЗХ — `sketch_drawing`, БПНХ — `draw_help`, СБК — `rough_blog`, награнь — `borderline`). 0 — это персональный блог (по умолчанию пост будет создан в нём).
...
* [ ] `pick` - путь к pick-файлу, временному файлу для выбора картинок (см. ниже про логику работы скрипта). Путь также относителен от домашней директории пользователя. Также можно указать здесь `'*:rentry'` для загрузки на [rentry.co](https://rentry.co) или `'*:dpaste'` для загрузки на [dpaste.com](https://dpaste.com).
...
Если `pick` начинается с символов `*:`, то это означает, что pick-файл будет залит на внешний ресурс, указанный после этих символов. Сейчас из таких ресурсов поддерживаются `rentry` и `dpaste`: если указать это слово после двоеточия, то pick-файл будет создан в формате Markdown и залит на выбранный ресурс.
