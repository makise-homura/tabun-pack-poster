...
Если же `pick` не начинается с этих символов, то pick-файл будет создан в формате HTML и положен по пути, указанному в переменной `pick` (относительно домашнего каталога пользователя).
